...
/* ENTRY POINT RETURNS AST OF SOURCE CODE */
...
/* PARSE LIST OF STATEMENTS AND ADD TO LIST */
AST_T* parser_parse_statements(parser_T* parser)
{
    ...
    /* WHILE WE HAVE SEMI COLONS WE CONTINUE PARSING CODE */
...
AST_T* parser_parse_variable_definition(parser_T* parser)
{
    ...
    AST_T* variable_definition_value = parser_parse_expression(parser); //expression

    AST_T* variable_definition = init_ast(AST_VARIABLE_DEFINITION);
    variable_definition->variable_name = variable_definition_variable_name;
...
AST_T* parser_parse_variable(parser_T* parser)
{
    ...
    {
        return parser_parse_function_call(parser); //if function parse function
    }
...
AST_T* parser_parse_id(parser_T* parser)
{
    if (
        ...
        strcmp(parser->current_token->value, "Array") == 0 || 
        strcmp(parser->current_token->value, "List") == 0 
        )
    {
